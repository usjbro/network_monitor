...
impl<'a> Decoder<'a> {
    ...
    fn update_max_dynamic_size(&mut self, buf: &[u8]) -> Result<usize, DecoderError> {
        // Upstream (0.3.1) called `.ok().unwrap()` here, panicking instead of
        // propagating a malformed/truncated dynamic-table-size-update
        // encoding as the `DecoderError` this function already returns for
        // every other failure mode. capture-agent's own fuzzing (issue #66)
        // found this: a crafted HEADERS frame reaches this line straight
        // from attacker-controlled decrypted TLS traffic (see http2.rs),
        // so this needed to become a normal `Err`, not a crash.
