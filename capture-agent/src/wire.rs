...
mod tests {
    ...
    #[test]
    fn packet_event_serializes_field_registry_and_header_dump_without_legacy_breakdown() {
        let event = AgentEvent::Packet {
            packet: Box::new(PacketJson {
                ...
                layer: 4,
                protocol: "TCP".into(),
                src: "192.168.1.10:51000".into(),
                dst: "93.184.216.34:80".into(),
                length: 60,
                summary: "TCP packet".into(),
                hex_dump: "00 01".into(),
        ...
        let encoded: serde_json::Value = serde_json::from_str(&encode_event(&event)).unwrap();
        let packet = &encoded["packet"];
        assert_eq!(packet["headerHexDump"], "aa bb cc");
        assert_eq!(packet["fields"][0]["path"], "tcp.src_port");
        assert_eq!(packet["fields"][0]["region"], "header");
        assert_eq!(packet["relativeTimeMs"], 1);
        assert!(packet.get("headerBreakdown").is_none());
    }
