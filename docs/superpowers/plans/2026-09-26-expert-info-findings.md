...
- [x] Write a test asserting `AgentEvent::Finding{..}` serializes to the exact JSON shape in the spec (snake_case Rust fields -> camelCase wire fields via existing `#[serde(rename_all = "camelCase")]`/`#[serde(tag = "type", rename_all = "snake_case")]` conventions), including optional `frame_id`/`flow_id` both present, both absent, and one-of-each.
- [x] Run it and confirm it fails (the type doesn't exist yet).
- [x] Add `Severity` (error/warning/note/chat) and `FindingCode` (retransmission/connection-reset/malformed-frame) enums with `#[serde(rename_all = "kebab-case")]` or explicit renames matching the spec's exact strings; add `FindingJson` and the `AgentEvent::Finding` variant.
...
- [x] Write `flow.rs` unit tests: a retransmitted segment makes `observe()` report `is_retransmit: true` exactly once (not on the original segment); a flow's first RST makes it report `rst_transitioned: true` exactly once, a second RST-flagged packet on the same already-reset flow reports `false`.
- [x] Run and confirm both fail against the current `Option<bool>` return type.
- [x] Change `observe()` to return `Option<ObserveResult { is_outbound: bool, is_retransmit: bool, rst_transitioned: bool }>` (or equivalent), updating `main.rs`'s one call site's destructuring accordingly. Every other `observe()` behavior (flow bookkeeping, direction) is unchanged.
- [x] Run flow.rs tests, confirm pass; run full `cargo test` to confirm the `main.rs` call-site update didn't regress anything else.
- [x] In `main.rs`'s capture loop, after the existing `packet_event_limiter.allow(now_ms)` check and `pkt-...` id generation (so a finding's `frameId` always corresponds to a real emitted `packet` event), emit a `retransmission` finding (severity `warning`, `frameId` = that packet's id) when `is_retransmit` was true.
- [x] Emit a `connection-reset` finding (severity `note`, `flowId` = `FlowKey::connection_id()`) when `rst_transitioned` was true — this one is *not* gated by the packet-event rate limiter (a flow's `connection_update` event has its own independent tick-based cadence, unrelated to per-packet limiting), so emit it right where `rst_transitioned` is read, not after the packet-event gate.
- [x] ~~Add a `main.rs`-level integration-style test~~ — deferred: main.rs's capture loop isn't unit-testable without a real device (only `tests/live_loopback.rs`'s `#[ignore]`d, capability-gated path reaches it), and the property holds by construction, not by runtime logic: the finding-emission code is textually placed after the `packet_event_limiter.allow(...)` branch's early `continue`, in the same block as the packet-event send it must stay consistent with — there is no code path that reaches the finding emission without having already reached the packet-id generation just above it. Extracted `malformed_frame_summary` as a pure, directly-unit-tested helper instead, covering the one piece of this integration point's logic that isn't already exercised elsewhere (Task 1's wire.rs tests cover `FindingJson` construction/serialization).
...
- [x] Emit the finding alongside the existing counter increment. Keep the summary observational: "N-byte frame did not decode as `<LinkType>` framing" — no claim about why.
- [x] Run and confirm pass; run the full Rust suite (`cargo test`, `cargo clippy --all-targets -- -D warnings`).
