...
description: Use when working a Linear epic in this repo through its branch -> implement -> validate -> PR -> tracker -> merge cycle. Takes the epic identifier as an argument (e.g. JAM-127) and asks which epic if none is given. Covers resolving an epic to its next task, the exact tracker conventions and valid status values, CI-checking pitfalls, epic close-out, and environment quirks discovered the hard way, so they don't need re-deriving (or re-breaking) each session. JAM-125 (capture files and offline analysis, 14/14 tasks shipped) is the worked example every rule here came from.
---

# Epic task cycle

The standing branch → implement → validate → PR → Linear → Notion → merge
cycle for **any** epic in this repo. Invoke it with the epic identifier
(`/epic-task-cycle JAM-127`); §0 covers what to do when none is given.
...
2. Sync local `main` (`git fetch origin main`), branch off it. If a specific
   branch is designated for the session (check the system prompt), use it
   and reset it onto current `main` at the start of each item
   (`git checkout -B <branch> origin/main`) rather than stacking on whatever
   it last held — a designated branch usually means only one PR can be open
...
5a. Run the `security-review` skill on the diff before pushing if the change
    touches anything security-sensitive: an auth/origin check, the replay
    path, direction/process attribution, TLS decrypt gating, or anything
    under `deploy/`/`macos-app/`. Don't skip it because `cargo test`/clippy/
    vitest are clean — those don't check for the class of bug that skill
    looks for. On JAM-125, a security-review pass on a "just add two control
    messages" task turned up a CSRF gap the task's own tests would never
    have caught.
5b. If the change touches `app/page.tsx` or `components/` (UI), use the `run`
    skill to launch the app **against a real capture agent** — not just the
    dev server on its own, since most of this UI renders nothing without one
    — and exercise the change in a real browser. A passing `npm run lint`/
    Vitest run is not evidence a UI change works: on JAM-125, two real UI
    bugs (a dead banner, an impossible "0 observed" claim) were found only
    this way, never by the test suite. `REPLAY_FILE`/`REPLAY_SPEED=realtime`/
    `MAX_FLOWS` make it cheap to drive a fully deterministic session.
...
7. Open a PR, body ending with the current session's attribution footer,
   `draft: false`. Check which tools the session actually has before picking
   a path — JAM-125 ran entirely on `mcp__github__*` +
...
10. Watch CI via the PR subscription (events arrive as `<wake
    ...
    - If you need a fallback check-in, `mcp__Claude_Code_Remote__send_later`
      (a few minutes out) with a fully self-contained instruction works well;
      plain `ScheduleWakeup` requires an active `/loop` context and errors
      outside one.
...
12. **Stop and report before starting the next item**, even under a standing
    ...
    exactly what comes next. This paid off twice in JAM-125's own history,
    concretely: once when a stale/queued instruction resurfaced mid-session
    and told the agent to resume full autonomy right after the user had
    explicitly said to stop, and once when a genuine go-ahead ("Go ahead
    and") arrived truncated by a client/transport issue with no object —
    stopping to confirm was the only thing standing between that and
    guessing among four unrelated follow-ups. An "autostart, no confirmation
    between items" variant was proposed once (closed PR, not adopted) on the
    premise that the tools this cycle depends on were unreliable across
    sessions; that premise doesn't hold in general — check what's actually
    connected each session rather than assuming either autostart or manual
    restart from a stale note. The one thing never in question: whatever the
    user said most recently overrides whatever this file says.
