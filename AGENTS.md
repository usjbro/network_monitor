...
- `next.config.ts` has a webpack tweak that disables file watching when `DISABLE_HMR=true` (used by the AI Studio agent environment to avoid flicker during automated edits) — don't "fix" the watchOptions block. Next.js 16 defaults to Turbopack, so `dev`/`build` in `package.json` pass `--webpack` explicitly to keep that hook applying. (The old `eslint.ignoreDuringBuilds` config key was removed in the Next 16 upgrade — no longer a recognized option; `npm run lint` remains the actual lint gate.)
