...
2. Approval can come from either channel, both handled by the same session that created the gate:
   - **In-chat**: when the human replies with approval in the same conversation, check `coordination/scripts/gate-status.sh <linear-id> <slug>` first. If still `status=awaiting-approval`, run `coordination/scripts/approve-gate.sh <linear-id> <slug>`, decide delegation via `coordination/router-checklist.md`, run it, then `coordination/scripts/mark-gate-delegated.sh <linear-id> <slug> <in-session|codex> [agent-type]`. If the status check shows it's already `approved`, it was already handled via a Slack reply your own polling picked up — report that instead of delegating again.
   - **Slack reply**: while your session remains active, you can poll `#network-monitor` yourself instead of only waiting on the next chat message — see `coordination/watcher-prompt.md` for the exact polling instructions (every 60 seconds, tracking last-read not last-sent). There is no separate scheduled/cloud watcher: cloud agents can't see local, uncommitted gate state, so this only runs inside the gate-creating session itself, and only while it's active.
